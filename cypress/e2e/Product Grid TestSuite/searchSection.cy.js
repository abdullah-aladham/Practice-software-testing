describe('Search Section Testsuite',()=>{
    ...
    it('[005-003-02] [invalid] trying to search with numerical data between 0-9',()=>{
        cy.get("#search-query").should('be.visible').and('not.be.disabled').and('be.null').type("0{enter}");
        //nothing should be happened
    })
    it('[005-003-03] [Valid] trying to search with string data[e.g :hammer]',()=>{
        cy.get("#search-query").should('be.visible').and('not.be.disabled').and('be.null').type("hammer{enter}");
        cy.intercept({
            method:"GET",
    ...
    it('[005-003-04] [Invalid] trying to search with less than 3 digit number',()=>{
        cy.get("#search-query").should('be.visible').and('not.be.disabled').and('be.null').type("1{enter}");
         
    })    
    		
    it('[005-003-05] [invalid] trying to search with less than 3 character string.',()=>{
        cy.get("#search-query").should('be.visible').and('not.be.disabled').and('be.null').type("he{enter}");
         
    })    
    
    it('[005-003-06][Valid] trying to search with irrelevant data to the product\business. (e.g:cola).',()=>{
        cy.get("#search-query").should('be.visible').and('not.be.disabled').and('be.null').type("cola{enter}");
